...
class Keithley_2400(Instrument):
	...
	def measure_voltage(self, nplc=1, voltage=21.0, auto_range=True):
		...
		if auto_range:
			...
			self._visainstrument.write(":SENS:VOLT:RANG:AUTO 0;:SENS:VOLT:RANG %g" % voltage)
		self.check_errors()
	...
	def measure_current(self, nplc=1, current=1.05e-4, auto_range=True):
		...
		if auto_range:
			...
			self._visainstrument.write(":SENS:CURR:RANG:AUTO 0;:SENS:CURR:RANG %g" % current)
	...
	def ramp_to_voltage(self, target_voltage, current_step):
		
		delay0 = .005
	   
			
		startV = self.get_voltage()
		
		len  = np.int(np.ceil(np.abs(target_voltage-startV)/current_step)+1)
		vList = np.linspace(startV, target_voltage, len)
	...
	def set_timed_arm(self, interval):
		...
		if interval > 99999.99 or interval < 0.001:
			raise Exception("Keithley 2400 can only be time triggered between 1 ms and 1 Ms")
