...
class QDevilQdac(Instrument):
    ...
    def __init__(self, name, port, verbose=False):
        ...
        self.voltageRange = {
            ch: 10.0 for ch in self.channelNumbers
        }  # Assumes that QDAC has power-on values
        self.currentRange = {
            ch: 100e-6 for ch in self.channelNumbers
        }  # Assumes that QDAC has power-on values
        self.triggerRange = range(0, 10)

        self.slowRate = 5e-3
        self.fastRate = 5e-1
        self.rates = np.zeros(24)

        self.add_function("close")
        self.add_function("flush")
        self.add_function("getSerialNumber")
        self.add_function("getNumberOfBoards")
        self.add_function("getNumberOfChannels")
        self.add_function("setVoltageRange")
        self.add_function("getVoltageRange")
        self.add_function("setCurrentRange")
        self.add_function("getCurrentRange")
        self.add_function("getDCVoltage")
        self.add_function("setDCVoltage")
        self.add_function("setRawDAC")
        self.add_function("getRawDAC")

        self.add_function("setChannelOutput")
        self.add_function("getChannelOutput")
        self.add_function("getCurrentReading")

        self.add_function("rampDCVoltage")
        self.add_function("allToZero")
    ...
    def setVoltageRange(self, channel, theRange):
        ...
        reply = self._checkForError(
            self._sendReceive(b"vol %d %d" % (channel, rangeFlag))
        )
    ...
    def setCurrentRange(self, channel, theRange):
        ...
        reply = self._checkForError(
            self._sendReceive(b"cur %d %d" % (channel, rangeFlag))
        ).decode("utf-8")
    ...
    def setRate(self, channel, fast):
        if fast:
            self.rates[channel - 1] = 1
        else:
            self.rates[channel - 1] = 0
    ...
    def setDCVoltage(self, channel, volts):
        ...
        reply = str(
            self._checkForError(
                self._sendReceive(b"set %d %e" % (channel, volts))
            ).decode("utf-8")
        )
        try:
    ...
    def rampDCVoltage(self, channel, volts):
        rate0 = self.slowRate
        delay0 = 0.005
        if self.rates[channel - 1] == 1:
            rate0 = self.fastRate

        startV = self.getDCVoltage(self, channel)

        len = np.int(np.ceil(np.abs(volts - startV) / rate0) + 1)
        vList = np.linspace(startV, volts, len)
    ...
    def setRawDAC(self, channel, dacValue):
        ...
        reply = str(
            self._checkForError(
                self._sendReceive(b"dac %d %d" % (channel, dacValue))
            ).decode("utf-8")
        )
        try:
            return int(
                reply.split("Digital Output:")[1].split("on Channel:")[0].strip(" ")
            )
        except:
            raise Exception("Error response from QDAC: <%s>" % reply)
    ...
    def setCalibrationChannel(self, channel):
        ...
        if self.debugMode == False:
            if channel != qdac.noChannel:
                self._validateChannel(channel)
        reply = self._checkForError(self._sendReceive(b"cal %d" % channel)).decode(
            "utf-8"
        )
    ...
    def readTemperature(self, board, position):
        ...
        if self.debugMode == False:
            if board not in [0, 1, 2, 3, 4, 5] or position not in [0, 1, 2]:
                raise Exception(
                    "readTemperature: Invalid board %d or position %d"
                    % (board, position)
                )
    ...
    def defineFunctionGenerator(
        self, generator, waveform, period, dutycycle=50, repetitions=-1, trigger=0
    ):
        ...
        if waveform == Waveform.sine:
            str = self._checkForError(
                self._sendReceive(
                    b"fun %d %d %f %d %d"
                    % (generator, waveform, period, repetitions, trigger)
                )
            ).decode("utf-8")
            period = float(
                str.split("Period:")[1].split("Repetitions")[0].replace(",", "")
            )
            returnValue.update({"Period": period})
            curveType = int(
                str.split("Curvetype:")[1].split("Period")[0].replace(",", "")
            )
        ...
        if waveform == Waveform.triangle or waveform == Waveform.square:
            str = self._checkForError(
                self._sendReceive(
                    b"fun %d %d %d %f %d %d"
                    % (generator, waveform, period, dutycycle, repetitions, trigger)
                )
            ).decode("utf-8")
            period = float(
                str.split("Period:")[1].split("Dutycycle")[0].replace(",", "")
            )
            dutyCycle = float(
                str.split("Dutycycle:")[1].split("Repetitions:")[0].replace(",", "")
            )
        ...
        if waveform == Waveform.staircase:
            ...
            stepLength = int(
                str.split("Step length:")[1].split("No. of steps")[0].replace(",", "")
            )
            returnValue.update({"Step length": stepLength})
            noOfSteps = int(
                str.split("No. of steps:")[1].split("Repetitions:")[0].replace(",", "")
            )
            returnValue.update({"No. of steps": noOfSteps})
            curveType = int(
                str.split("Curvetype:")[1].split("Step length")[0].replace(",", "")
            )

        repetitionRate = int(
            str.split("Repetitions:")[1].split("Trigger")[0].replace(",", "")
        )
    ...
    def getFunctionGenerator(self, generator):
        if not self.debugMode:
            if generator not in Generator.functionGenerators:
                raise Exception(
                    "Invalid generator number (must be 1-8): %d" % generator
                )
        str = self._checkForError(self._sendReceive(b"fun %d" % generator)).decode(
            "utf-8"
        )
        curveType = ""
        if "Period" in str:
    ...
    def definePulsetrain(
        self, lowDuration, highDuration, lowVolts, highVolts, repetitions=-1, trigger=0
    ):
        ...
        highValue = float(
            reply.split("high value:")[1].split()[0].strip(",").strip("V")
        )
        pulseCount = int(
            reply.split("high value:")[1].split("times")[0].split(",")[1].strip(" ")
        )
    ...
    def defineAWG(self, samples, repetitions=-1, trigger=0):  # Sample rate is 1kS/s
        ...
        for idx in range(0, len(samples), 64):
            cmd = (
                "awg 0 0 " + " ".join(["%e" % v for v in samples[idx : idx + 64]])
            ).encode("ascii")
            self._sendReceive(cmd)
        reply = str(
            self._checkForError(
                self._sendReceive(b"run %d %d" % (repetitions, trigger))
            ).decode("utf-8")
        )
        returnValue = {}
        samples = int(
            reply.split("Samples:")[1].split(", Repetitions:")[0].strip(" ").strip(",")
        )
        repetitions = int(
            reply.split("Repetitions:")[1].split("Trigger:")[0].strip(" ").strip(",")
        )
        trigger = int(reply.split("Trigger:")[1].strip(" ").strip("'"))
        returnValue.update(
            {"Samples": samples, "Repetitions": repetitions, "Trigger": trigger}
        )
        return returnValue
    ...
    def setChannelOutput(self, channel, generator, amplitude=1.0, offset=0):
        ...
        return self._checkForError(
            self._sendReceive(
                b"wav %d %d %e %e" % (channel, generator, amplitude, offset)
            )
        ).decode("utf-8")
    ...
    def getChannelOutput(self, channel):
        if not self.debugMode:
            self._validateChannel(channel)
        reply = str(
            self._checkForError(self._sendReceive(b"wav %d" % channel)).decode("utf-8")
        )
        generator = int(
            reply.split("Generator:")[1].split("Amplitude")[0].replace(",", "")
        )
        amplitude = float(
            reply.split("Amplitude:")[1].split("Offset")[0].replace(",", "")
        )
    ...
    def setSyncOutput(self, syncChannel, generator, delay=0, pulseLength=1):
        ...
        res = self._checkForError(
            (
                self._sendReceive(
                    b"syn %d %d %d %d" % (syncChannel, generator, delay, pulseLength)
                )
            )
        )
        syncCh = int(
            res.split(b"bound to generator")[0]
            .strip(b"Sync output")
            .strip(b" ")
            .strip(b",")
        )
        generator = int(
            res.split(b"bound to generator")[1]
            .split(b"delay")[0]
            .strip(b" ")
            .strip(b",")
        )
    ...
    def setSyncOutputOff(self, syncChannel):
        ...
        if syncChannel not in self.syncChannels:
            raise Exception("Invalid sync channel (must be 0-5): %d" % syncChannel)
        res = self._checkForError(
            self._sendReceive(b"syn %d %d %d %d" % (syncChannel, 0, 0, 0))
        )
        syncCh = int(
            res.split(b"bound to generator")[0]
            .strip(b"Sync output")
    ...
    def setVoltageCalibration(self, channel, theRange, samplesPerVolt, offsetSamples):
        ...
        return self._checkForError(
            self._sendReceive(
                b"vcal %d %d %e %e"
                % (channel, rangeFlag, samplesPerVolt, offsetSamples)
            )
        ).decode("utf-8")
    ...
    def getVoltageCalibration(self, channel, theRange):
        ...
        if theRange == 10.0:
            ...
            raise Exception("Invalid voltage range %f" % theRange)
        reply = self._checkForError(
            self._sendReceive(b"vcal %d %d" % (channel, rangeFlag))
        ).decode("utf-8")
        return {
            "Gain": float(reply.split("mult")[1].split("offset")[0].strip()),
            "Offset": float(reply.split("offset")[1].strip()),
        }

    def setCurrentCalibration(
        self, channel, theRange, microampsPerSample, offsetMicroamps
    ):
        # range must be 1e-6 or 100e-6 (unit is A)
        # Note: Data is not stored in persisted memory. When instrument is rebooted, it is set back to factory values.
        # To store permanently, call storeCalibrationInFlash
        ...
        return self._checkForError(
            self._sendReceive(
                b"ical %d %d %e %e"
                % (channel, rangeFlag, microampsPerSample, offsetMicroamps)
            )
        ).decode("utf-8")
    ...
    def getCurrentCalibration(self, channel, theRange):
        ...
        if theRange == 1e-6:
            ...
            raise Exception("Invalid current range: %e" % theRange)
        reply = self._checkForError(
            self._sendReceive(b"ical %d %d" % (channel, rangeFlag))
        ).decode(
            "utf-8"
        )  # TODO: Proper formatting processing of response
        return {
            "Gain": float(reply.split("mult")[1].split("offset")[0].strip()),
            "Offset": float(reply.split("offset")[1].strip()),
        }
    ...
    def setBaudrate(self, baudrate):
        ...
        if baudrate in BaudratesAllowed:
            reply = self._checkForError(self._sendReceive(b"brt %d" % baudrate)).decode(
                "utf-8"
            )
    ...
    def getMinMaxOutputVoltage(self, channel, theRange):
        ...
        if theRange == 10.0:
            ...
            raise Exception("Invalid voltage range: %f" % theRange)
        reply = self._checkForError(
            self._sendReceive(b"rang %d %d" % (channel, rangeFlag))
        ).decode("utf-8")
        return {
            "MinVoltage": float(reply.split("MIN:")[1].split("MAX")[0].strip()),
    ...
    def executeTrigger(self, triggerNumber):
        ...
        return self._checkForError(
            self._sendReceive(b"trig %d" % (triggerNumber))
        ).decode("utf-8")
    ...
    def linkExternalTrigger(self, triggerNumber, value=1):
        if not self.debugMode:
            self._validateTrigger(triggerNumber)
        reply = self._checkForError(
            self._sendReceive(b"trig %d %d" % (triggerNumber, value))
        ).decode("utf-8")
    ...
    def syncPortOut(self, port="A", enabledisable=1):
        if port == "A":
            ...
            if enabledisable == 1 or enabledisable == 0:
                self._checkForError(
                    self._sendReceive(b"synB %d" % (enabledisable))
                ).decode("utf-8")
            else:
                raise Exception("Invalid enabledisable value: %d" % enabledisable)
    ...
    def setExternalSampleClock(self, enabledisable):
        if enabledisable == 1 or enabledisable == 0:
            return self._checkForError(
                self._sendReceive(b"extclk %d" % (enabledisable))
            ).decode("utf-8")
        else:
            raise Exception("Invalid enabledisable value: %d" % enabledisable)
    ...
    def _validateVoltage(self, channel, volts):
        if (
            volts < 0.001 - self.voltageRange[channel]
            or volts > self.voltageRange[channel] - 0.001
        ):
