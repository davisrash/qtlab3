...
def create_data(filename, lockins, vectors, num_gates, names):
    ...
    vectors : list[np.ndarray[float]]
        A list of vectors representing data for the channel parameters
        X and Y.
...
class S1():
    ...
    def qdac_1gate(self,channel,xname,xstart,xend,xstep,rev,threshold,compliance):
        ...
        for x in x_vector:
            ...
            if threshold is not None : 
                ...
                if (threshold_set==0): 
                    if( datavalues[13] > threshold or datavalues[13] < 0):
                        print('conductance threshold stop')
                        break
                elif (threshold_set==1): 
                    if( np.abs(datavalues[15]) < threshold ):
                        print('current threshold stop')
                        break
            
            if( datavalues[2*channel-1] > compliance):
                print('compliance stop')
                break    
            x1_vector.append(x)
        '''
        Find ith peak, assign the voltage to a variable, return the variable
        '''
        data_fwd._write_settings_file()                                                                                                             # Overwrite the settings file created at the beginning, this ensures updating the sweep variable with the latest value
        data_fwd.close_file()
        print('scan done')
    ...
    def record(self,channel, voltage, time0, timestep):
        ...
        while (wait<5):        #so magnet doesn't time out, we measure it every second
            #B_now = magnet.get_fieldZ()
            wait = wait + 1
            qt.msleep(delay)
