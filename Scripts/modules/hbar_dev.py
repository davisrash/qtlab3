...
def create_data(filename, instruments, circuit, sweeps):
    ...
    if isinstance(sweeps, dict):
        sweeps = [
            sweeps,
            {
                "name": "empty, you should fix me! code is ugly :(",
                "start": 0.0,
                "stop": 0.0,
                "vector": [0.0],
            },
    ...
    for lockin in instruments["lockins"]:
        for dim in ["x", "y"]:
            data.add_value(
                lockin.get_name() + " " + dim + " raw",
                instrument=lockin.get_name(),
                units="V",
            )
            data.add_value(
                lockin.get_name() + " " + dim + " pros",
                instrument=lockin.get_name(),
                units="V",
            )
...
def gate_sweep(filename, instruments, circuit, sweeps, channels=None):
    ...
    for sweep in sweeps:
        sweep["vector"] = np.linspace(
            sweep["start"],
            sweep["stop"],
            int(np.ceil(np.abs((sweep["stop"] - sweep["start"]) / sweep["step"])) + 1),
        )
...
def magnet_sweep(filename, instruments, circuit, sweeps, channels=None):
    ...
    for magnet in instruments["magnets"]:
        ...
        sweeps["step"] = (
            int(
                np.abs((sweeps["stop"] - sweeps["start"]))
                / sweeps["ramp_rate"]
                * 60
                / sweeps["intrasweep_delay"]
            )
            + 5
        )

        sweeps["vector"] = np.linspace(sweeps["start"], sweeps["stop"], sweeps["step"])
    ...
    for magnet in instruments["magnets"]:
        ...
        print(
            "Start Bnow: {}, Bend: {}, gate voltage: {}".format(
                sweeps["start"], sweeps["stop"], sweeps["v_gate"]
            )
        )
