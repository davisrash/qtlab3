...
#import timetrack
import sys
sys.path.append('C:\Users\physics-svc-laroche\Desktop\qtlab-master\script\modules')
import numpy as np
import data as d
import traces
import shutil
...
class S1():
    ...
    def qdac_1gate(self,channel,xname,xstart,xend,xstep,rev,threshold,compliance):
        ...
        for x in x_vector:
            ...
            qt.msleep(delay2)  
            datavalues = self.take_data(x)                                                                                                 # Go to next sweep value and take data
            data_fwd.add_data_point(x,0,0,datavalues[0],datavalues[1],datavalues[3],datavalues[3],datavalues[4],datavalues[5],datavalues[6],datavalues[7],datavalues[8],datavalues[9],datavalues[10],datavalues[11],datavalues[12],datavalues[13],datavalues[14])                                                                     # write datapoint into datafile
                                                                                                                                            # Record data in memory
        ...
        if (rev):
            ...
            for x1 in x1_vector:
                ...
                qt.msleep(delay2)   
                datavalues = self.take_data(x1)                                                                                                 # Go to next sweep value and take data
                data_bck.add_data_point(x,0,0,datavalues[0],datavalues[1],0,0,datavalues[4],datavalues[5],datavalues[6],datavalues[7],datavalues[8],datavalues[9],datavalues[10],datavalues[11],datavalues[12],datavalues[13],datavalues[14])                                                                   # write datapoint into datafile
                                                                                                                                                # Record data in memory
    ...
    def qdac_2gate(self,channel,channel2,xname1,xstart1,xend1,xstep1,xname2,xstart2,xend2,xstep2,threshold,compliance):
        ...
        for x1 in x_vector1:
            ...
            for x2 in x_vector2:
                ...
                #if( qdac1.getDCVoltage(1) > 1):  # threshold on what the gate leakage is reading and fill with zeros after
...
a.qdac_1gate(1,'gate_right',start1,end1,xstep1,rev,threshold,compliance)
